...
class TestApp:
    ...
    def checkSquareForms(self, data_Json, squares, id):
        ...
        for innerObj in data_Json:
            self.checkForm(innerObj,  'http://localhost:8083/Square' + str(squares[square_index]) + "/" + id, 'application/json', 'PUT', 'writeproperty')
    ...
    def test_index(self, client):
        ...
        assert data_Json["ttt:Agent"] == 'http://localhost:8083/basicbot'
       
        for innerObj in data_Json["forms"]:
            self.checkForm(innerObj,  'http://localhost:8083/register', 'application/json', 'POST', 'writeproperty')
    ...
    def test_register(self, client):
        ...
        self.checkType(res.json, "ttt:Game")
        ...
        self.checkLink(res.json["links"][1], "http://localhost:8083/Board/" + id, "GET")
        data_Json = res.json
    ...
    def test_index_registered(self, client):
        ...
        self.checkLink(data_Json["links"][0], "http://localhost:8083/Board/" + id, "GET")
    ...
    def test_register_twice(self, client):
        ...
        self.checkLink(res.json["links"][1], "http://localhost:8083/Board/" + id2, "GET")
        assert id != id2
    ...
    def test_square_form(self, client):
        ...
        self.checkLink(data_Json["links"][1], "http://localhost:8083/Board/" + id, "GET")
    ...
    def test_board_no_moves(self, client):
        ...
        res = client.get("Board/" + id)
        assert res.status_code == 200
        data = res.get_data()
        data_Json = json.loads(data)
        
        # Check id, type, context
        self.checkID(data_Json,"http://localhost/Board/" + id)
        self.checkContext(data_Json)
    ...
    def test_board_made_one_move(self, client):
        ...
        self.checkLink(res_square.json["links"][1], "http://localhost:8083/Board/" + id, "GET")
        
        # GET board response
        res_board = client.get("Board/" + id)
        assert res_board.status_code == 200
        data_board = res_board.get_data()
        data_board = json.loads(data_board)
        
        # Check id, type, context
        self.checkID(data_board,"http://localhost/Board/" + id)
        ...
        # Should have forms to all squares except Square11,and Square12 (as basicBOT has made this move) and should not have a form to register
    ...
    def test_board_result_page(self, client):
        ...
        #Get the board, get links - assume basicBot strategy is to take next free 
        # square (12) - FIXME: these tests will break if have a different bot strategy
        id = self.extractID(res_square.json["links"][1])
        self.checkLink(res_square.json["links"][1], "http://localhost:8083/Board/" + id, "GET")
        
        # PUT to square form 21
        res_square = client.put("Square21/" + id)
        assert res_square.status_code == 200
         # Not expecting any errors
        with pytest.raises(KeyError):
            res_square.json["sch:error"]
        # ... and 31
        res_square = client.put("Square31/" + id)
        assert res_square.status_code == 200
         # Not expecting any errors
        with pytest.raises(KeyError):
            res_square.json["sch:error"]
        ...
        self.checkLink(data["links"][1], "http://localhost:8083/Board/" + id, "GET")
        self.checkLink(data["links"][2], "http://localhost:8083/result/" + id, "GET")
        # Should not have any forms
        with pytest.raises(KeyError):
            data["forms"] 

        # Get the results link
        results = client.get("result/" + id)
        data = results.json
        ...
        self.checkLink(data["links"][0], "http://localhost:8083/Board/" + id, "GET")
