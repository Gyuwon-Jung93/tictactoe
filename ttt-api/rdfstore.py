...
class RDFStore():
    ...
    def __init__(self, graph, prefix, url, xagentplayer, oagentplayer, id):
        ...
        game_instance = URIRef(url + "Game/"+id)
        ...
        board_instance = URIRef(url + "Board/"+id)
        ...
        for (s, p, o) in self.graph.triples((None, RDFS.subClassOf, self.ttt.Square)):
            ...
            square_url = url + "Square"+str(square_int)+"/"+id
    ...
    def get_role(self, agent):
        if (agent == self.xagent):
            return self.ttt.XPlayerRole
        if (agent == self.oagent):
            return self.ttt.OPlayerRole
    ...
    def add_move(self, square, agent):
        ...
        self.graph.add((square,  self.ttt.moveTakenBy, self.get_role(agent)))
    ...
    def get_winner(self):
        ...
        for agent in player_1:
            if (self.check_for_win(player_1[agent])):
                self.graph.add((self.game, self.ttt.hasResult, agent))
                return agent
